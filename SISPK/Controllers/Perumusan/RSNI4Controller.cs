...
{
    [Auth(RoleTipe = 1)]
    public class RSNI4Controller : Controller
    {
        ...
        public ActionResult Pengesahan(string PROPOSAL_ABSTRAK = "", int PROPOSAL_ID = 0, int PROPOSAL_KOMTEK_ID = 0, string PROPOSAL_NO_SNI_PROPOSAL = "", string PROPOSAL_JUDUL_SNI_PROPOSAL = "", string POLLING_START_DATE = "", string POLLING_END_DATE = "", int APPROVAL_TYPE = 1, string NO_RAKOR = "", string TGL_RAKOR = "")
        {
            ...
            var VERSION_RAKOR = db.Database.SqlQuery<decimal>("SELECT CAST(NVL(MAX(T1.PROPOSAL_RAPAT_VERSION),0) AS NUMBER)+1 AS PROPOSAL_RAPAT_VERSION FROM TRX_PROPOSAL_RAPAT T1 WHERE T1.PROPOSAL_RAPAT_PROPOSAL_ID = " + PROPOSAL_ID + " AND T1.PROPOSAL_RAPAT_PROPOSAL_STATUS = 12").SingleOrDefault();
            ...
            {
                Directory.CreateDirectory(Server.MapPath("~/Upload/Dokumen/RANCANGAN_SNI/RSNI4/" + PROPOSAL_PNPS_CODE_FIXER));
                string path = Server.MapPath("~/Upload/Dokumen/RANCANGAN_SNI/RSNI4/" + PROPOSAL_PNPS_CODE_FIXER + "/");
                ...
                if (EXT_SURAT_UNDANGAN_RAPAT.ToLower() == ".docx" || EXT_SURAT_UNDANGAN_RAPAT.ToLower() == ".doc")
                {
                    ...
                    var FVALUE_SURAT_UNDANGAN_RAPAT = "'" + LASTID_SURAT_UNDANGAN_RAPAT + "', " +
                                ...
                                "'43', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") Surat Undangan Rapat Koordinasi RSNI 4 Ver " + VERSION_RAKOR + "', " +
                ...
                {
                    FILE_SURAT_UNDANGAN_RAPAT.SaveAs(path + "SURAT_UNDANGAN_RAPAT_RSNI4_Ver_" + VERSION_RAKOR + "_" + PROPOSAL_PNPS_CODE_FIXER + EXT_SURAT_UNDANGAN_RAPAT.ToLower());
                    int LASTID_SURAT_UNDANGAN_RAPAT = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_SURAT_UNDANGAN_RAPAT = MixHelper.GetLogCode();
                    var FNAME_SURAT_UNDANGAN_RAPAT = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_SURAT_UNDANGAN_RAPAT = "'" + LASTID_SURAT_UNDANGAN_RAPAT + "', " +
            ...
            if (FILE_BERITA_ACARA.ContentLength > 0)
            {
                ...
                if (EXT_BERITA_ACARA.ToLower() == ".docx" || EXT_BERITA_ACARA.ToLower() == ".doc")
                {
                    ...
                    var FVALUE_BERITA_ACARA = "'" + LASTID_BERITA_ACARA + "', " +
                                ...
                                "'42', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") Berita Acara RSNI 4 Ver " + VERSION_RAKOR + "', " +
                ...
                {
                    FILE_BERITA_ACARA.SaveAs(path + "BERITA_ACARA_RSNI4_Ver_" + VERSION_RAKOR + "_" + PROPOSAL_PNPS_CODE_FIXER + EXT_BERITA_ACARA.ToLower());
                    int LASTID_BERITA_ACARA = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_BERITA_ACARA = MixHelper.GetLogCode();
                    var FNAME_BERITA_ACARA = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_BERITA_ACARA = "'" + LASTID_BERITA_ACARA + "', " +
            ...
            if (FILE_DAFTAR_HADIR.ContentLength > 0)
            {
                ...
                if (EXT_DAFTAR_HADIR.ToLower() == ".docx" || EXT_DAFTAR_HADIR.ToLower() == ".doc")
                {
                    ...
                    var FVALUE_DAFTAR_HADIR = "'" + LASTID_DAFTAR_HADIR + "', " +
                                ...
                                "'16', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") Daftar Hadir Rapat Koordinasi RSNI 4 Ver " + VERSION_RAKOR + "', " +
                ...
                {
                    FILE_DAFTAR_HADIR.SaveAs(path + "DAFTAR_HADIR_RSNI4_Ver_" + VERSION_RAKOR + "_" + PROPOSAL_PNPS_CODE_FIXER + EXT_DAFTAR_HADIR.ToLower());
                    int LASTID_DAFTAR_HADIR = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_DAFTAR_HADIR = MixHelper.GetLogCode();
                    var FNAME_DAFTAR_HADIR = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_DAFTAR_HADIR = "'" + LASTID_DAFTAR_HADIR + "', " +
            ...
            if (FILE_NOTULEN.ContentLength > 0)
            {
                ...
                if (EXT_NOTULEN.ToLower() == ".docx" || EXT_NOTULEN.ToLower() == ".doc")
                {
                    ...
                    var FVALUE_NOTULEN = "'" + LASTID_NOTULEN + "', " +
                                ...
                                "'44', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") Notulen Rapat Koordinasi RSNI 4 Ver " + VERSION_RAKOR + "', " +
                ...
                {
                    FILE_NOTULEN.SaveAs(path + "NOTULEN_RSNI4_Ver_" + VERSION_RAKOR + "_" + PROPOSAL_PNPS_CODE_FIXER + EXT_NOTULEN.ToLower());
                    int LASTID_NOTULEN = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_NOTULEN = MixHelper.GetLogCode();
                    var FNAME_NOTULEN = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_NOTULEN = "'" + LASTID_NOTULEN + "', " +
            ...
            HttpPostedFileBase FILE_SURAT_JP = Request.Files["SURAT_JP"];
            if (FILE_SURAT_JP.ContentLength > 0)
            {
                Directory.CreateDirectory(Server.MapPath("~/Upload/Dokumen/RANCANGAN_SNI/RSNI4/" + PROPOSAL_PNPS_CODE_FIXER));
                string path = Server.MapPath("~/Upload/Dokumen/RANCANGAN_SNI/RSNI4/" + PROPOSAL_PNPS_CODE_FIXER + "/");
                Stream STREAM_DOC_SURAT_JP = FILE_SURAT_JP.InputStream;

                string EXT_SURAT_JP = Path.GetExtension(FILE_SURAT_JP.FileName);
                if (EXT_SURAT_JP.ToLower() == ".docx" || EXT_SURAT_JP.ToLower() == ".doc")
                {
                    ...
                    doc.Save(@"" + filePathpdf, Aspose.Words.SaveFormat.Pdf);
                    doc.Save(@"" + filePathxml);
                    int LASTID_SURAT_JP = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_SURAT_JP = MixHelper.GetLogCode();
                    var FNAME_SURAT_JP = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_SURAT_JP = "'" + LASTID_SURAT_JP + "', " +
                                "'15', " +
                                "'45', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") Surat Jajak Pendapat Ulang " + VERSION_RAKOR + "', " +
                                ...
                                "'1', " +
                                "'" + LOGCODE_SURAT_JP + "'";
                    db.Database.ExecuteSqlCommand("INSERT INTO TRX_DOCUMENTS (" + FNAME_SURAT_JP + ") VALUES (" + FVALUE_SURAT_JP.Replace("''", "NULL") + ")");
                    String objekTanggapan = FVALUE_SURAT_JP.Replace("'", "-");
                    MixHelper.InsertLog(LOGCODE_SURAT_JP, objekTanggapan, 1);
                }
                else
                {
                    FILE_SURAT_JP.SaveAs(path + "SURAT_JP_RSNI4_Ver_" + VERSION_RAKOR + "_" + PROPOSAL_PNPS_CODE_FIXER + EXT_SURAT_JP.ToLower());
                    int LASTID_SURAT_JP = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_SURAT_JP = MixHelper.GetLogCode();
                    var FNAME_SURAT_JP = "DOC_ID,DOC_FOLDER_ID,DOC_RELATED_TYPE,DOC_RELATED_ID,DOC_NAME,DOC_DESCRIPTION,DOC_FILE_PATH,DOC_FILE_NAME,DOC_FILETYPE,DOC_EDITABLE,DOC_CREATE_BY,DOC_CREATE_DATE,DOC_STATUS,DOC_LOG_CODE";
                    var FVALUE_SURAT_JP = "'" + LASTID_SURAT_JP + "', " +
            ...
            if (FILE_DATA_RSNI.ContentLength > 0)
            {
                ...
                if (EXT_DATA_RSNI.ToLower() == ".docx" || EXT_DATA_RSNI.ToLower() == ".doc")
                {
                    ...
                    var FVALUE_DATA_RSNI = "'" + LASTID_DATA_RSNI + "', " +
                                ...
                                "'17', " +
                                "'" + PROPOSAL_ID + "', " +
                                "'" + "(" + PROPOSAL_PNPS_CODE_FIXER + ") RSNI 4', " +
            ...
            if (APPROVAL_TYPE == 1)
            {
                ...
                db.Database.ExecuteSqlCommand("UPDATE TRX_MONITORING SET MONITORING_TGL_AWAL_JP_2 = " + POLLING_START_DATE_CONVERT + ", MONITORING_TGL_AKHIR_JP_2 = " + POLLING_START_DATE_CONVERT + " WHERE MONITORING_PROPOSAL_ID = " + PROPOSAL_ID);

                var NEW_LASTID_POLLING = db.Database.SqlQuery<int>("SELECT POLLING_ID FROM TRX_POLLING WHERE POLLING_PROPOSAL_ID =" + PROPOSAL_ID + " AND POLLING_VERSION = " + (LAST_POLLING_VERSION + 1) + " AND POLLING_TYPE = 12").SingleOrDefault();
                db.Database.ExecuteSqlCommand("UPDATE TRX_PROPOSAL SET PROPOSAL_STATUS = 13, PROPOSAL_STATUS_PROSES = 1,PROPOSAL_ABSTRAK = '" + PROPOSAL_ABSTRAK + "', PROPOSAL_IS_POLLING = 1, PROPOSAL_NO_SNI_PROPOSAL = '" + PROPOSAL_NO_SNI_PROPOSAL + "', PROPOSAL_JUDUL_SNI_PROPOSAL = '" + PROPOSAL_JUDUL_SNI_PROPOSAL + "',PROPOSAL_JUDUL_PNPS = '" + PROPOSAL_JUDUL_SNI_PROPOSAL + "', PROPOSAL_POLLING_ID = " + NEW_LASTID_POLLING + ", PROPOSAL_UPDATE_DATE = " + DATENOW + ", PROPOSAL_UPDATE_BY = " + USER_ID + " WHERE PROPOSAL_ID = " + PROPOSAL_ID);
                ...
                String objek1 = "UPDATE TRX_PROPOSAL SET PROPOSAL_STATUS = 13,PROPOSAL_STATUS_PROSES = 1, PROPOSAL_IS_POLLING = 1, PROPOSAL_NO_SNI_PROPOSAL = '" + PROPOSAL_NO_SNI_PROPOSAL + "', PROPOSAL_JUDUL_SNI_PROPOSAL = '" + PROPOSAL_JUDUL_SNI_PROPOSAL + "', PROPOSAL_POLLING_ID = " + NEW_LASTID_POLLING + ", PROPOSAL_UPDATE_DATE = " + DATENOW + ", PROPOSAL_UPDATE_BY = " + USER_ID + " WHERE PROPOSAL_ID = " + PROPOSAL_ID;
                MixHelper.InsertLog(PROPOSAL_LOG_CODE, objek1.Replace("'", "-"), 2);

                var APPROVAL_REASON = "";
                db.Database.ExecuteSqlCommand("UPDATE TRX_PROPOSAL_APPROVAL SET APPROVAL_STATUS = 1 WHERE APPROVAL_PROPOSAL_ID = " + PROPOSAL_ID);
                var APPROVAL_STATUS_SESSION = db.Database.SqlQuery<decimal>("SELECT CAST(NVL(MAX(T1.APPROVAL_STATUS_SESSION),0) AS NUMBER) AS APPROVAL_STATUS_SESSION FROM TRX_PROPOSAL_APPROVAL T1 WHERE T1.APPROVAL_PROPOSAL_ID = " + PROPOSAL_ID + " AND T1.APPROVAL_STATUS_PROPOSAL = 12").SingleOrDefault();
                db.Database.ExecuteSqlCommand("INSERT INTO TRX_PROPOSAL_APPROVAL (APPROVAL_ID,APPROVAL_PROPOSAL_ID,APPROVAL_TYPE,APPROVAL_REASON,APPROVAL_DATE,APPROVAL_BY,APPROVAL_STATUS,APPROVAL_STATUS_PROPOSAL,APPROVAL_STATUS_SESSION) VALUES (" + APPROVAL_ID + "," + PROPOSAL_ID + ",0,'" + APPROVAL_REASON + "'," + DATENOW + "," + USER_ID + ",1,12," + APPROVAL_STATUS_SESSION + ")");
            ...
            else
            {
                ...
                var APPROVAL_STATUS_SESSION = db.Database.SqlQuery<decimal>("SELECT CAST(NVL(MAX(T1.APPROVAL_STATUS_SESSION),0) AS NUMBER) AS APPROVAL_STATUS_SESSION FROM TRX_PROPOSAL_APPROVAL T1 WHERE T1.APPROVAL_PROPOSAL_ID = " + PROPOSAL_ID + " AND T1.APPROVAL_STATUS_PROPOSAL = 12").SingleOrDefault();
                db.Database.ExecuteSqlCommand("INSERT INTO TRX_PROPOSAL_APPROVAL (APPROVAL_ID,APPROVAL_PROPOSAL_ID,APPROVAL_TYPE,APPROVAL_REASON,APPROVAL_DATE,APPROVAL_BY,APPROVAL_STATUS,APPROVAL_STATUS_PROPOSAL,APPROVAL_STATUS_SESSION) VALUES (" + APPROVAL_ID + "," + PROPOSAL_ID + ",1,'" + APPROVAL_REASON + "'," + DATENOW + "," + USER_ID + ",1,12," + APPROVAL_STATUS_SESSION + ")");
            }
            TempData["Notifikasi"] = 1;
            TempData["NotifikasiText"] = "Data Berhasil Disimpan";
            return RedirectToAction("Index");
        }
        public ActionResult DataRSNI4(DataTables param)
        {
