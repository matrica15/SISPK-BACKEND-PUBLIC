...
namespace SISPK.Controllers.Perumusan
{
    ...
    public class VerifikasiController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Pengesahan(TRX_PROPOSAL tp, int PROPOSAL_ID = 0, int PROPOSAL_KOMTEK_ID = 0, string PROPOSAL_PNPS_CODE = "", int APPROVAL_TYPE = 0, int APPROVAL_STATUS = 1)
        {
            var USER_ID = Convert.ToInt32(Session["USER_ID"]);
            ...
            if (FILE_DATA_RSNI.ContentLength > 0)
            {
                ...
                if (EXT_DATA_RSNI.ToLower() == ".docx" || EXT_DATA_RSNI.ToLower() == ".doc")
                {
                    ...
                    string filePathdoc = path + "DATA_RASNI_Ver_1_" + PROPOSAL_PNPS_CODE_FIXER + ".docx";
                    string filePathpdf = path + "DATA_RASNI_Ver_1_" + PROPOSAL_PNPS_CODE_FIXER + ".pdf";
                    string filePathxml = path + "DATA_RASNI_Ver_1_" + PROPOSAL_PNPS_CODE_FIXER + ".xml";
                    ...
                    var FVALUE_DATA_RSNI = "'" + LASTID_DATA_RSNI + "', " +
                                ...
                                "'" + "DATA_RASNI_Ver_1_" + PROPOSAL_PNPS_CODE_FIXER + "', " +
                ...
                {
                    FILE_DATA_RSNI.SaveAs(path + "DATA_RASNI_Ver_1_" + PROPOSAL_PNPS_CODE_FIXER + EXT_DATA_RSNI.ToLower());
                    int LASTID_DATA_RSNI = MixHelper.GetSequence("TRX_DOCUMENTS");
                    var LOGCODE_DATA_RSNI = MixHelper.GetLogCode();
            ...
            String objek1 = "UPDATE TRX_PROPOSAL SET PROPOSAL_STATUS = 15, PROPOSAL_STATUS_PROSES = 1, PROPOSAL_IS_POLLING = NULL, PROPOSAL_POLLING_ID = NULL, PROPOSAL_UPDATE_DATE = " + DATENOW + ", PROPOSAL_UPDATE_BY = " + USER_ID + " WHERE PROPOSAL_ID = " + PROPOSAL_ID;
            MixHelper.InsertLog(PROPOSAL_LOG_CODE, objek1.Replace("'", "-"), 2);
            int APPROVAL_ID = MixHelper.GetSequence("TRX_PROPOSAL_APPROVAL");
            var APPROVAL_STATUS_SESSION = db.Database.SqlQuery<decimal>("SELECT CAST(NVL(MAX(T1.APPROVAL_STATUS_SESSION),0) AS NUMBER) AS APPROVAL_STATUS_SESSION FROM TRX_PROPOSAL_APPROVAL T1 WHERE T1.APPROVAL_PROPOSAL_ID = " + PROPOSAL_ID + " AND T1.APPROVAL_STATUS_PROPOSAL = 14").SingleOrDefault();
            db.Database.ExecuteSqlCommand("INSERT INTO TRX_PROPOSAL_APPROVAL (APPROVAL_ID,APPROVAL_PROPOSAL_ID,APPROVAL_TYPE,APPROVAL_DATE,APPROVAL_BY,APPROVAL_STATUS,APPROVAL_STATUS_PROPOSAL,APPROVAL_STATUS_SESSION) VALUES (" + APPROVAL_ID + "," + PROPOSAL_ID + ",1," + DATENOW + "," + USER_ID + ",1,14," + APPROVAL_STATUS_SESSION + ")");


            //------------------------------------
            TempData["Notifikasi"] = 1;
            TempData["NotifikasiText"] = "Data Berhasil Disimpan dan Diteruskan ke Usulan Penetapan";
        ...
        public ActionResult DataRASNIPPS(DataTables param)
        {
            ...
            string where_clause = "PROPOSAL_STATUS = 14 AND PROPOSAL_STATUS_PROSES = 1  " + ((BIDANG_ID != 0) ? "AND KOMTEK_BIDANG_ID IN (" + BIDANG_ID + ",0)" : "");
                         ...
                         select new string[] 
            {
                ...
                Convert.ToString("<center>"+list.PROPOSAL_TAHAPAN+"</center>"),
                Convert.ToString("<center>"+list.PROPOSAL_STATUS_NAME+"</center>"),
                Convert.ToString("<center><a href='/Perumusan/Verifikasi/Detail/"+list.PROPOSAL_ID+"' class='btn blue btn-sm action tooltips' data-container='body' data-placement='top' data-original-title='Lihat'><i class='action fa fa-file-text-o'></i></a>"+((list.PROPOSAL_STATUS == 14 && list.PROPOSAL_STATUS_PROSES == 1)?"<a href='/Perumusan/Verifikasi/Pengesahan/"+list.PROPOSAL_ID+"' class='btn purple btn-sm action tooltips' data-container='body' data-placement='top' data-original-title='Pengesahan RASNI'><i class='action fa fa-check'></i></a>":"")+"<a href='javascript:void(0)' onclick='cetak_usulan("+list.PROPOSAL_ID+")' class='btn green btn-sm action tooltips' data-container='body' data-placement='top' data-original-title='Cetak'><i class='action fa fa-print'></i></a></center>"),
