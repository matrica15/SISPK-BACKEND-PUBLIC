...
namespace SISPK.Controllers.Document
{
    public class DocumentTypeController : Controller
    {
        ...
        public ActionResult ListData(DataTables param, int status)
        {
            ...
            string where_clause = "REF_TYPE = 2 AND REF_STATUS = '" + status + "' ";
                         ...
                         select new string[] 
            {
                ...
                Convert.ToString((status == 1) ? "<a data-original-title='Ubah' data-placement='top' data-container='body' class='btn purple btn-sm action tooltips' href='/Document/DocumentType/Edit/"+list.REF_ID+"'><i class='action fa fa-edit'></i></a><a data-original-title='Non Aktifkan' data-placement='top' data-container='body' class='btn red btn-sm action tooltips' href='/Document/DocumentType/Delete/"+list.REF_ID+"'><i class='action glyphicon glyphicon-remove'></i></a></center>":"") +
                Convert.ToString((status == 0) ? "<a data-original-title='Aktifkan' data-placement='top' data-container='body' class='btn green btn-sm action tooltips' href='/Document/DocumentType/Activate/"+list.REF_ID+"'><i class='action glyphicon glyphicon-check'></i></a></center>":"") +
                Convert.ToString("<center>")
            };
            return Json(new
            {
        ...
        public ActionResult Edit(FormCollection formCollection)
        {
            ...
            {
                string RefId = formCollection["DOCTYPE_ID"];
                string RefName = formCollection["DOCTYPE_NAME"];

                string query_reset = "UPDATE MASTER_REFERENCES SET REF_NAME = '" + RefName + "' WHERE REF_TYPE = 2 AND REF_ID = " + RefId;
        ...
        {
            string query_update_group = "UPDATE MASTER_REFERENCES SET REF_STATUS = 0 WHERE REF_ID = '" + id + "' AND REF_TYPE = 2";
        ...
        {
            string query_update_group = "UPDATE MASTER_REFERENCES SET REF_STATUS = 1 WHERE REF_ID = '" + id + "' AND REF_TYPE = 2";
            db.Database.ExecuteSqlCommand(query_update_group);
            TempData["Notifikasi"] = 1;
