...
namespace SISPK.Controllers.WebServices
{
    public class SNIAPIController : Controller
    {
        ...
        public ActionResult getICSList(
            string username = "",
            string password = "",
            string proposal_id = "",
            string keyword = "aa",
        ...
        )
        {
            ...
            var validate = ValidateAccountParam(username, password, proposal_id);
            ...
            else
            {
                ...
                if (username == "sni" && password == "sni123")
                {
                    ...
                    var start = Convert.ToInt32(limit) * (Convert.ToInt32(page) - 1);
                    //var ProposalLists = db.Database.SqlQuery<VIEW_SNI>("SELECT * FROM VIEW_SNI WHERE ROWNUM <= 400");
                    var ICS = db.Database.SqlQuery<VIEW_PROPOSAL_ICS>("SELECT * FROM VIEW_PROPOSAL_ICS WHERE PROPOSAL_ICS_REF_PROPOSAL_ID =" + proposal_id);
