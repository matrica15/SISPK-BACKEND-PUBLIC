...
{
    [Auth(RoleTipe = 1)]
    public class KomtekController : Controller
    {
        //
        // GET: /Komtek/
        ...
        public ActionResult Create(MASTER_KOMITE_TEKNIS m_komtek, FormCollection formCollection, int[] KOMTEK_ICS_ID )
        {
            ...
            //var ics_id = m_komtek.KOMTEK_ICS_ID.ToString();
            ...
            return RedirectToAction("CreateAnggota/" + idk);
        }
        ...
        public ActionResult DetailKomtek(int id = 0) 
        {
            ...
            ViewData["komtek_item"] = (from t in db.VIEW_KOMTEK where t.KOMTEK_ID == id select t).SingleOrDefault();
            ...
            //VIEW_KOMTEK komtek_item = db.VIEW_KOMTEK.SingleOrDefault(t => t.KOMTEK_ID == id);
        ...
        public ActionResult CreateAnggota(int id = 0) {
        ...
        [HttpPost]
        public ActionResult CreateAnggota(MASTER_KOMTEK_ANGGOTA mka)
        {            
            var UserId = Session["USER_ID"];
            var logcode = MixHelper.GetLogCode();
            ...
            TempData["Notifikasi"] = 1;
            TempData["NotifikasiText"] = "Data Berhasil Disimpan dan informasi user telah dikirim ke email" + mka.KOMTEK_ANGGOTA_EMAIL;
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            ViewData["komtek_item"] = (from t in db.VIEW_KOMTEK where t.KOMTEK_ID == id select t).SingleOrDefault();
            ViewData["komtek_ics"] = (from t in db.VIEW_KOMTEK_ICS where t.KOMTEK_ICS_KOMTEK_ID == id && t.KOMTEK_ICS_STATUS == 1 select t).ToList();
            var file = (from t in db.TRX_DOCUMENTS where t.DOC_FOLDER_ID == 22 && t.DOC_RELATED_ID == id && t.DOC_STATUS == 1 select t).SingleOrDefault();
            ViewData["sk_file"] = ((file == null)?null:file);
            //return Json(new
            //{
            //    sEcho = file
        ...
        public ActionResult Nonaktif(int id = 0)
        {
            ...
            if (count_parent == 0)
            {
                ...
                db.Database.ExecuteSqlCommand("UPDATE MASTER_KOMITE_TEKNIS SET KOMTEK_STATUS = 0 WHERE KOMTEK_ID = " + id);
                db.Database.ExecuteSqlCommand("UPDATE MASTER_KOMITE_TEKNIS SET KOMTEK_STATUS = 0 WHERE KOMTEK_PARENT_CODE = '" + km.KOMTEK_CODE+"'");
            }
            TempData["Notifikasi"] = 1;
            TempData["NotifikasiText"] = "Data Berhasil Di Non Aktifkan";
            return RedirectToAction("Index");
        ...
        public ActionResult ListDataKomtek(DataTables param, int status = 0) { 
            ...
            { 
                ...
                Convert.ToString((list.KOMTEK_STATUS == 1)?"<center><a data-original-title='Add anggota' data-placement='top' data-container='body' class='btn blue btn-sm action tooltips' href='/Master/Komtek/CreateAnggota/"+list.KOMTEK_ID+"'><i class='action fa fa-users'></i></a><a data-original-title='Detail' data-placement='top' data-container='body' class='btn blue btn-sm action tooltips' href='/Master/Komtek/DetailKomtek/"+list.KOMTEK_ID+"'><i class='action fa fa-file-text-o'></i></a><a data-original-title='Ubah' data-placement='top' data-container='body' class='btn purple btn-sm action tooltips' href='/Master/Komtek/Edit/"+list.KOMTEK_ID+"'><i class='action fa fa-edit'></i></a>"+"<a data-original-title='Non-Aktifkan' data-placement='top' data-container='body' class='btn red btn-sm action tooltips' href='/Master/Komtek/Nonaktif/"+list.KOMTEK_ID+"'><i class='action glyphicon glyphicon-remove'></i></a></center>":"<center><a data-original-title='Add anggota' data-placement='top' data-container='body' class='btn blue btn-sm action tooltips' href='/Master/Komtek/CreateAnggota/"+list.KOMTEK_ID+"'><i class='action fa fa-users'></i></a><a data-original-title='Detail' data-placement='top' data-container='body' class='btn blue btn-sm action tooltips' href='/Master/Komtek/DetailKomtek/"+list.KOMTEK_ID+"'><i class='action fa fa-file-text-o'></i></a>"+"<a data-original-title='Ubah' data-placement='top' data-container='body' class='btn purple btn-sm action tooltips' href='/Master/Komtek/Edit/"+list.KOMTEK_ID+"'><i class='action fa fa-edit'></i></a>"+"<a data-original-title='Hapus' data-placement='top' data-container='body' class='btn green btn-sm action tooltips' href='/Master/Komtek/Aktif/"+list.KOMTEK_ID+"'><i class='action glyphicon glyphicon-ok'></i></a></center>"),
            };
            return Json(new
            {
                sEcho = param.sEcho,
        ...
        public ActionResult getics(string q = "")
        {
            ...
            foreach (var listField in SelectedData)
            {
                results.Add(new
                {
                    ...
                    name_ics_ind = ((listField.ICS_NAME_IND == null) ? listField.ICS_NAME.ToString() : listField.ICS_NAME_IND.ToString())
                });
            }
